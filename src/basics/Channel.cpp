...
Channel::Channel(int _fd, EventLoop* _loop, bool _is_io_channel) : fd(_fd), idx(ch_deleted), loop(_loop), listen_events(0), ready_events(0), addedToLoop(false), is_io_channel(_is_io_channel), eventHandling(false), tied(false) {}
...
bool Channel::tied_() { return tied; }
void Channel::setIndex(int _idx) { idx = _idx; }
void Channel::set_io_channel() { is_io_channel = true; }
...
void Channel::reset_callbacks() { in_callback = out_callback = pri_callback = err_callback = rdhup_callback = hup_callback = NULL; }
