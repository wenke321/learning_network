...
class Buffer
{
   public:
    ...
    Buffer(size_t initSize = InitialSize) : buf(initSize), read_idx(0), write_idx(0)
    {
    ...
    Buffer(Buffer&& _other)
    {
        ...
        write_idx = _other.write_idx;
        read_idx  = _other.read_idx;
    }
    ...
    // after read all
    void retrieveAll() { read_idx = write_idx = 0; }
    ...
    // void append(const std::string& _str) { append(_str.data(), _str.size()); }
   ...
   private:
    ...
    void addCap(size_t len)
    {
        ...
        memmove(buf.data(), buf.data() + read_idx, readable);
        read_idx  = 0;
        write_idx = readable;
    }
