...
std::string sockOption::resolve_domain_ipv4(const char* domain)
{
    ...
    {
        {
            LOG_SYSERR << "getaddrinfo error: %s\n" << gai_strerror(status);
        }
        return "";
    }

    // 3. 遍历结果链表，提取第一个可打印的 IP
    for (p = res; p != NULL; p = p->ai_next)
    {
        ...
        {  // IPv4
            ...
            addr                     = &(ipv4->sin_addr);
            inet_ntop(p->ai_family, addr, (char*)ip.data(), ip.max_size());
            break;  // 只取第一个结果
        }
        // else
        // {  // IPv6
...
int sockOption::getSocketError(int sockfd)
{
    ...
    {
        LOG_ERROR << " ::getsockopt,errno=" << errno << ",fd=" << sockfd;
        return -1;
    }
    return op;
}
...
int sockOption::get_keep_alive(int sockfd)
{
    int op      = 0;
    socklen_t l = sizeof(op);

    if (::getsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, &op, &l) < 0)
    {
